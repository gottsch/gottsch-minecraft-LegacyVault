...
public class VaultScreen extends AbstractContainerScreen<VaultContainerMenu> {
    ...
    protected void renderLabels(PoseStack matrixStack, int mouseX, int mouseY) {
		...
		this.font.draw(matrixStack, new TranslatableComponent(LangUtil.screen("vault.name")).getString(), LABEL_XPOS, CHEST_LABEL_YPOS, Color.DARK_GRAY.getRGB());

		String vaultsRemaining = "";

		if (ServerConfig.COMMUNITY.enabled.get()) {
			vaultsRemaining = new TranslatableComponent(LangUtil.screen("community_vault")).getString();
		}
			...
			if (!ServerConfig.PERSONAL.unlimitedVaults.get()) {
				...
				vaultsRemaining = new TranslatableComponent(LangUtil.screen("vaults_remaining"), String.valueOf(ServerConfig.PERSONAL.vaultsPerPlayer.get() - cap.getCount()), ServerConfig.PERSONAL.vaultsPerPlayer.get()).getString();
			}
			else {
				vaultsRemaining = new TranslatableComponent(LangUtil.screen("unlimited_vaults")).getString();
			}
		}
